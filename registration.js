...
function phoneDisplay(value){
  ...
  return digits.replace(/[0-9]/g, d => '০১২৩৪৫৬৭৮৯'[d]);
}
function isFirebaseReady(){
  return typeof firebase !== 'undefined' && typeof db !== 'undefined' && !!firebase.apps?.length;
}
async function saveRegistration(ref, payload, attempts=2){
  let lastError;
  for(let i=0;i<attempts;i++) {
    try {
      const write = ref.set(payload);
      return await Promise.race([
        write,
        new Promise((_, reject)=>setTimeout(()=>reject(new Error('WRITE_TIMEOUT')), 15000))
      ]);
    } catch(error) {
      lastError = error;
...
form.addEventListener('submit', async (event)=>{
  ...
  if(!isFirebaseReady()){
    statusBox.textContent='❌ Firebase সংযোগ প্রস্তুত নয়। firebase-config.js যাচাই করুন।';
    statusBox.style.color='#c62828';
    return;
  }
  submitBtn.disabled=true;
  submitBtn.innerHTML='জমা দেওয়া হচ্ছে <span>…</span>';
  statusBox.textContent='তথ্য নিরাপদে সংরক্ষণ করা হচ্ছে…';
  statusBox.style.color='#08733d';
  if(!pendingRegistrationId){
    pendingRegistrationId = `REG-${Date.now()}-${Math.random().toString(36).slice(2,7).toUpperCase()}`;
  }
  const registrationId = pendingRegistrationId;
  const ref = db.collection('registrations').doc(registrationId);
  try{
    await saveRegistration(ref, payload, 2);
    applicationData={...payload,id:registrationId,createdAt:new Date()};
    ...
    console.error(error);
    const message = error?.message === 'WRITE_TIMEOUT'
      ? '⏳ সার্ভারে উত্তর পেতে বেশি সময় লাগছে। ইন্টারনেট ঠিক আছে কিনা দেখে আবার চেষ্টা করুন।'
      : '❌ তথ্য জমা দেওয়া যায়নি। Firebase সেটআপ, Firestore Rules ও ইন্টারনেট সংযোগ যাচাই করুন।';
